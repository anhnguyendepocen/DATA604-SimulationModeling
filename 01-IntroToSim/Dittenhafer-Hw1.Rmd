...
inventorySim <- function(daysToSimulate, daysPerWeek)
{
  ...
  for(i in seq(1, nrow(simTable)))
  {
    ...
    else
    {
      ...
      simTable[i,]$shortage <- if(endI < 0) simTable[i-1,]$shortage + simTable[i,]$demand - simTable[i,]$beginInv else 0
      
      # Ordering
      if(simTable[i,]$endInv <= 10 && simTable[i-1,]$pendingOrder == 0)
      {
...
daysToSim <- 5 * 5
inventorySimTable <- inventorySim(daysToSim, 5)
```
