...
if(!file.exists(filename))
{
  ...
  bmMil <- bmstats(m)
  arMil <- arstats(m)
  dfAll <- cbind(method=rep("itstats", m), itMil)
  dfAll <- rbind(dfAll, cbind(method=rep("bmstats", m), bmMil))
  dfAll <- rbind(dfAll, cbind(method=rep("arstats", m), arMil))
...
g58 <- ggplot(dfAll) + geom_histogram(aes(x=r, colour=method)) + myTheme + facet_wrap(~ method)
g58
```
...
# Show me the results of the Monte Carlo simulations
head(dfMcPi)
# What is the median pi estimate?
median(pi_est)
# What is the mean pi estimate?
mean(pi_est)
```

```{r, echo=FALSE}
g1 <- ggplot(data=dfMcPoints) + geom_point(aes(x=x, y=y, color=inside))
g1
```
